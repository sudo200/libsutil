...
void *arraylist_get(arraylist *restrict list, size_t index) {
  if (list == NULL) {
    errno = EINVAL;
    return NULL;
  }
...
size_t arraylist_length(arraylist *restrict list) {
  if (list == NULL) {
    errno = EINVAL;
    return 0UL;
  }
...
int arraylist_foreach(arraylist *restrict list, void (*cb)(void *, void *), void *pipe) {
...
void **arraylist_to_array(arraylist *restrict list) {
  if (list == NULL) {
    errno = EINVAL;
    return NULL;
  }
