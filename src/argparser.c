                          ...
                          char longopt_seperator, const char *parse_stop)
{
  ...
  for(*optint = 1; *optint < argc; (*optint)++)
  {
    ...
    if(startswith(argv[*optint], shortopt_start) && !startswith(argv[*optint], longopt_start)) // short option
    {
      ...
      if(strlen(opt_str) == 0 && (*optint >= argc || startswith(argv[*optint + 1], longopt_start) || startswith(argv[*optint + 1], shortopt_start)))
        return ARGPARSE_ARG_REQ;

      msprintf(opt->argument, "%s", strlen(opt_str) > 1 ? opt_str + 1 : argv[++(*optint)]);
    }
